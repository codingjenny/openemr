...
class FhirQuestionnaireResponseService extends FhirServiceBase implements IResourceReadableService, IResourceSearchableService, IResourceCreatableService
{
    ...
    public function insert(FHIRDomainResource $fhirResource): ProcessingResult
    {
        ...
        // Route to mapped services
        $services = $this->getMappedServices();
        if (!empty($services)) {
            foreach ($services as $service) {
                if ($service instanceof IResourceCreatableService) {
                    return $service->insert($fhirResource);
                }
            }
        }
        
        // If no mapped service can handle it, return an error
        $processingResult = new ProcessingResult();
        $processingResult->setValidationMessages(['No service found to handle QuestionnaireResponse insert']);
    ...
    /**
     * Parses a FHIR resource into an OpenEMR record array.
     * This is a facade service that routes to mapped services.
     */
    public function parseFhirResource(FHIRDomainResource $fhirResource): array
    {
        // This method is handled by mapped services
        return [];
    }
    ...
    public function insertOpenEMRRecord($openEmrRecord): ProcessingResult
    {
        // This method is handled by mapped services
        $processingResult = new ProcessingResult();
        $processingResult->setValidationMessages(['insertOpenEMRRecord should not be called directly on facade service']);
        return $processingResult;
    }
    ...
    public function updateOpenEMRRecord($fhirResourceId, $updatedOpenEMRRecord): ProcessingResult
    {
        // This method is handled by mapped services
        $processingResult = new ProcessingResult();
        $processingResult->setValidationMessages(['Update not yet implemented for QuestionnaireResponse']);
        return $processingResult;
    }

    /**
     * Searches for OpenEMR records.
     * Routes to mapped services.
     */
    public function searchForOpenEMRRecords($openEMRSearchParameters): ProcessingResult
    {
        return $this->searchAllServicesWithSupportedFields($openEMRSearchParameters);
    }

    /**
     * Parses an OpenEMR record into a FHIR resource.
     * This is a facade service that routes to mapped services.
     */
